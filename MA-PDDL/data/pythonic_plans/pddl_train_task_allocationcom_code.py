...
# We can perform SubTask 1 and SubTask 2 in parallel, and Subtask 3 after Subtask 1 and 2 is done.

# pddl problem file 
# SubTask 1: Turn off the light

(define (problem turn-off-light)
  ...
  (:goal
    (switch-off allactionrobot light)
    (not (inaction allactionrobot))  
  )
)
...
(define (problem turn-on-faucet)
  ...
  (:goal
    (not (inaction allactionrobot))  
    (switch-on allactionrobot faucet)
  )
)
...
# Task Allocation Rules: Each subtask should be assigned to a robot or allactionrobot that collectively possess all the skills required for the subtask and can handle the mass of the objects involved in the task. If a subtask cannot be performed by a single robot due to its skill set or mass capacity,
...
# problem file solution done.
#IMPORTANT strictly follow the structure, stop generate beyond problem file solution done.
...
(define (problem slice-the-potato)
  (:domain allactionrobot)
  (:objects
    allactionrobot - robot
    potato - object
    kitchen-counter - object  
  )

  (:init
    (not (inaction allactionrobot)) 
    (at-location kitchen-counter potato)  
  )
  (:goal
    (not (inaction allactionrobot))  
    (sliced allactionrobot potato)
  )
)
...
# robot Allactionrobot are required since SubTasks can't be performed with individual robots as explained above. The 'Slice the Potato' subtask is assigned to allactionrobot. 
